...
interface AgentClientChatProps {
...
export const AgentClientChat: React.FC<AgentClientChatProps> = ({ interactionChats, listings, humanTakeoverChats, onTakeoverChat, onSendAgentMessage }) => {
    const activeChats = Object.entries(interactionChats).filter(([, messages]: [string, Message[]]) => messages.length > 0);
        ...
        <div className="bg-white dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-lg p-6 animate-fade-in-up">
            ...
            {activeChats.length === 0 ? (
                ...
                <div className="space-y-6 max-h-[60vh] overflow-y-auto pr-2">
                    {/* FIX: Explicitly type `messages` as `Message[]` to prevent it from being inferred as `unknown`. */}
                    {activeChats.map(([propertyId, messages]: [string, Message[]]) => {
